...
import { Resizer1 as styles } from './styles';
...
class Resizer1 extends Component {
  constructor(props) {
    super(props);
  ...
  validateWidth(newWidth) {
    const { offsetWidth, offsetHeight } = this.child.current;
    const newHeight = this.props.symmetric ? newWidth * offsetHeight / offsetWidth : null;
    return validateSize(newWidth, newHeight, this.maxWidth);
  }

  validateHeight(newHeight) {
    const { clientWidth: width, clientHeight: height } = this.child.current;
  ...
  startHorizontalResizing(event) {
    ...
    this.original = {
      pageX: event.pageX,
      width: this.child.current.offsetWidth,
      height: this.child.current.offsetHeight
    };
  ...
  horizontalResize(event) {
    ...
    const newHeight = this.original.height * newWidth / this.original.width;
    if (this.validateWidth(newWidth)) this.child.current.style.width = newWidth + 'px';
    if (this.validateHeight(newHeight)) this.child.current.style.height = newHeight + 'px';
  }
  ...
  startVerticalResizing(event) {
    ...
    this.original = {
      ...
      height: this.child.current.clientHeight
    };
  ...
  stopVerticalResizing(event) {
    ...
    this.props.onResize?.({
      width: this.child.current.clientWidth,
      height: this.child.current.clientHeight
    });
  }
...
export default withStyles(styles)(Resizer1);